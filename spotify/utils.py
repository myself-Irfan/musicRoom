...
def get_usr_tokens(session_key):
    print(f'Searching for {session_key}')
...
def update_or_create_usr_token(session_key, access_token, token_type, expires_in, refresh_token):
    ...
    try:
        ...
        if created:
            print(f'A new SpotifyToken was created for {session_key}')
        else:
...
def is_spotify_authenticated(session_key):
    ...
    if token:
        ...
        if expiry <= timezone.now():
            ...
            if not new_token:
                print(f'Failed to refresh spotify token for session key {session_key}')
                return False
        print('Token is yet to expire')
        return True

    print(f'No spotify token found for the provided session key {token}')
    return False
...
def execute_spotify_api_request(session_key, endpoint, post_=False, put_=False):
    ...
    if not token:
        ...
        print(f'Found Token: {token.access_token}')
...
def pause_song(session_id):
    print('Will request to pause')
    return execute_spotify_api_request(session_id, "player/pause", put_=True)


def play_song(session_id):
    print('Will request to play')
    return execute_spotify_api_request(session_id, "player/play", put_=True)


def skip_song(session_id):
    print('Will request to skip')
